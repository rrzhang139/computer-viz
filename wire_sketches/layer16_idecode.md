...
bit-fields — `rs1`, `rs2`, `rd`, `opcode`. The whole 8-bit word arrives as one
bus (`/8`); the decoder splits it into four 2-bit fields, each leaving as its
own bus (`/2`). Because we chose the opcode encoding to match the ALU's `op`
encoding, **no logic is needed** — the split is pure field extraction.

| instruction field | bits | datapath input |
|-------------------|------|----------------|
| opcode            | 2    | op             |
| rs1               | 2    | raddrA         |
| rs2               | 2    | raddrB         |
...
| key        | role                            | (x, y)     | edge   |
|------------|---------------------------------|------------|--------|
| instr_in   | 8-bit instruction word (/8)     | (-9,  0)   | LEFT   |
| op_out     | opcode field → ALU op (/2)      | ( 9,  4.5) | RIGHT  |
