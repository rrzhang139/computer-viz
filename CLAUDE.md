...
- **The `animations/` track is gated by `npm test` (from `animations/`), which is machine-enforced — don't bypass it.** It runs, in order: `spatial-conventions` (terminal-edge invariant) → `check.mjs --all` (Manhattan/box/orphan wireframe geometry) → `wireframe-gate` (page ↔ wireframe ↔ data-net) → `decoder-1to1` → **`hover-preview` (every drillable block — `cursor:pointer` `[id^="slot-"]` — MUST reveal the child's internals on hover, CLAUDE.md rule 21)** → `drilldown`. When you add a layer/primitive, these fail until the layer conforms; that is the enforcement, not reviewer goodwill. The hover-preview gate in particular exists because the connected-hover preview is the easiest thing to forget when a page still "works" on click. Pages and slots are auto-discovered, so a new `*.html` / `slot-*` is covered without editing the test.
