...
1. **Spatial invariant** (locked, machine-enforced): LEFT = inputs (data AND control), RIGHT = outputs, TOP = Vdd / supply, BOTTOM = GND. [BRACKETS] = one unit of this layer. `...` = zoom-in marker. The rule is enforced by `animations/tests/spatial-conventions.test.mjs`, wired as a `prewireframes` prehook in `animations/package.json` — every wireframe edit runs the check before `render.mjs` re-emits the SVGs. A `*_in` terminal anywhere but LEFT, or `*_out` anywhere but RIGHT, fails the test. Pre-rule legacy exceptions (CLK_in on TOP for clocked sequentials) live in a frozen `LEGACY_ALLOWLIST` inside the test; adding to it requires a code edit + justification, not just a wireframe change. See also `INVARIANTS.md`.
...
23. **Interactive prose walkthrough pattern** (`animations/` pages; `cpu.html` is the reference, machine-enforced by `tests/prose-highlight.test.mjs`): the side panel holds `.step` articles driven by ONE shared implementation — `src/steps.ts` (nav, gates, focus, state sync) + `src/proseHighlight.ts` (click-to-spotlight). Per step, opt-in via data attributes:
    - `data-stage="fetch"` — deep-link name (`?stage=fetch`).
    - `data-focus="#gPc,#gImem"` — camera zooms to those elements when the step shows (`all` = fit).
    - `data-gate="#btnStep"` — "next →" soft-locks until the reader uses that control.
    - `data-pc="1"` — the page's `onStepShow` hook pins sim state so the diagram always matches the prose.
    - Each narrative paragraph carries `data-hl="net:X open:ID box:ID pin:ID"`; clicking it spotlights exactly what it names (named wires flash, `open:` blocks pop their live preview, everything else dims). Every token MUST resolve to real canvas elements — a dead token fails the gate. A spotlit wire carrying 0 shows muted in its hue (spotlit ≠ lit). When a paragraph names a wire, also name the wire's endpoint block (`box:`/`open:`) so the destination isn't dimmed away.
    - `data-hl-focus` (optional, same `<p>`) — while the spotlight is active the camera zooms to the spotlit elements (or to an explicit `"#a,#b"` list); clearing restores the step's `data-focus`. Use when a paragraph reads preview internals that are illegible at step zoom.
    - Prose color-references use the `.wt-<net>` classes emitted by `applyWireColors` so a term's tint always matches its wire.

## Testing discipline (every change)

- **The `animations/` track is gated by `npm test` (from `animations/`), which is machine-enforced — don't bypass it.** It runs, in order: `spatial-conventions` (terminal-edge invariant) → `check.mjs --all` (Manhattan/box/orphan wireframe geometry) → `wireframe-gate` (page ↔ wireframe ↔ data-net) → `decoder-1to1` → **`hover-preview` (every drillable block — `cursor:pointer` `[id^="slot-"]` — MUST reveal the child's internals on hover, CLAUDE.md rule 21)** → **`wire-alignment` (every parent wire endpoint coincides with the child preview's terminal)** → **`preview-fidelity`** → **`prose-highlight` (every `data-hl` paragraph spotlights real elements; gates hold; `data-pc` state matches the prose — CLAUDE.md rule 23)** → `drilldown`. When you add a layer/primitive, these fail until the layer conforms; that is the enforcement, not reviewer goodwill. Pages and slots are auto-discovered, so a new `*.html` / `slot-*` is covered without editing the test.
- **CANONICAL preview rule (machine-enforced by `preview-fidelity`): a drillable block's hover preview MUST be an EXACT, live copy of the page you drill into — not a hand-drawn approximation.** Mark the slot `data-embed-page="/child.html" data-embed-svg="<svgId>"`; `src/embedPreview.ts`'s `autoFillEmbeds(svg)` clones that page's real `<svg>` into the slot's `.detailed` (single source via Vite `?raw`) and returns the child's projected pin positions; route every parent wire ONTO those pins, and light the embedded wires/bodies (`data-body`) to match state. `preview-fidelity.test.mjs` then enforces, per `[data-embed-page]` slot: (a) FIDELITY — preview contains every `data-net` the target page's static SVG has; (b) ALIGNMENT — every parent-wire endpoint inside the box lands on an embedded pin and every pin has a wire (no lines to empty space); (c) READABILITY — min on-screen gap between distinct-net wires ≥ 2.5px, with a screenshot saved to `test-results/preview-fidelity/`. **Every new drillable preview must use this mechanism.** (`datapath.html` is the reference; the older scene-builder/inline minis on other pages are being migrated to it.)
